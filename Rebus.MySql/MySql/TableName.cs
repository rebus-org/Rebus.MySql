...
/// <summary>
/// Represents a (possibly schema-qualified) table name in MySQL
/// </summary>
public class TableName : IEquatable<TableName>
{
    ...
    /// <summary>
    /// Returns the qualified name given the Schema and Name
    /// </summary>
    ...
    public TableName(string schema, string tableName)
    {
        ...
        Schema = StripBackticks(schema);
        Name = StripBackticks(tableName);
    }

    /// <summary>
    /// Parses the given name into a <see cref="TableName"/>, defaulting to using the default connection schema unless the name is schema-qualified.
    /// E.g. 'table' will result in a <see cref="TableName"/> representing the `table' table, whereas 'accounting.messages' will
    ...
    public static TableName Parse(string name)
    {
        ...
        else
        {
            ...
            var nameWithoutOutermostTicks = name.Substring(1, name.Length - 2);
            ...
            var parts = Regex.Split(nameWithoutOutermostTicks, @"\`[ ]*\.[ ]*\`");
    ...
    static string StripBackticks(string value)
    {
        if (value.StartsWith("`"))
        {
