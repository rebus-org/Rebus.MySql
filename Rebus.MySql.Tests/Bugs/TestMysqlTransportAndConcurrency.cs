...
[TestFixture]
public class TestMysqlTransportAndConcurrency : FixtureBase
{
    [TestCase(2, 2, 10)]
    [TestCase(5, 10, 100)]
    ...
    [Repeat(10)]
    [Description(@"Tried (without success) to reproduce a situation where the same message seemed to be received twice, allegedly because of MySQL failing to properly handling the exclusive lock&update required to effectively grab a lease on the message. 
...
It did not look like this was the case though, as everything seemed to work just like it should.")]
    public async Task TryToReproduceTheIssue(int numberOfBusInstances, int parallelismPerInstance, int messageCount, int timeoutSeconds = 5)
    {
        ...
        Using(new DisposableCallback(() => MySqlTestHelper.DropTable(inputQueueName)));
